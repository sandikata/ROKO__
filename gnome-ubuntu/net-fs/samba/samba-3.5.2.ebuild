...
src_install() {
	...
	# install misc files
	insinto /etc/samba
	newins "${CONFDIR}"/smb.conf.default smb.conf
...
pkg_postinst() {
	...
	# Fixing ownership and permissions on usershare dir
	chown root:${USERSHARES_GROUP} "${ROOT}"/${USERSHARES_DIR}
	chmod 01770 "${ROOT}"/${USERSHARES_DIR}

	einfo "Users who are to be allowed to use usershare should be added"
