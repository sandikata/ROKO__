...
src_prepare() {
	...
	# use_dbus_service allow 'Lock Screen' button of gnome-extra/indicator-session working
	cd "${WORKDIR}"
	epatch ${PN}_${PV}-0ubuntu1.diff
	cd "${S}"
	epatch debian/patches/{02_keep_unlock_raised,11_autostart_delay,12_use_dbus_service,10_legacy_scrsvr_inhibit}.patch
...
src_install() {
	...
	doins debian/org.gnome.ScreenSaver.service
