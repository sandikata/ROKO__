...
if isdarwin && [[ -d /opt/local ]]; then
    ...
    # zshenv.
    PATH="/opt/local/bin:/opt/local/sbin:/opt/bin:/opt/sbin:/home/ROKO__/bin:/opt/sopcast-bin:$PATH"
...
if [[ "$TERM" == dumb ]] ; then
    PROMPT="${EXITCODE}${debian_chroot:+($debian_chroot)}%n@%m %{$fg[green]%}[ %* ] [ %W ] %{$fg[cyan]%}[ %l ]%{$reset_color%}% %40<...<%B%~%b%<< "
else
...
${BLUE}%n${NO_COLOUR}@%m %{$fg[green]%}[ %* ] [ %W ] %{$fg[cyan]%}[ %l ] %{$reset_color%}% %40<...<%B%~%b%<< "
    else
        # This assembles the primary prompt string
        if (( EUID != 0 )); then
            PROMPT="${RED}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}${BLUE}%n${NO_COLOUR}@%m %{$fg[green]%}[ %* ] [ %W ] %{$fg[cyan]%}[ %l ] %{$reset_color%}% %40<...<%B%~%b%<< "
        else
            PROMPT="${BLUE}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}${RED}%n${NO_COLOUR}@%m %{$fg[green]%}[ %* ] [ %W ] %{$fg[cyan]%}[ %l ] %{$reset_color%}% %40<...<%B%~%b%<< "
        fi
    fi
fi
...
if [[ -n "$GRML_CHROOT" ]] ; then
    PROMPT="%{$fg[red]%}(CHROOT) %{$fg_bold[red]%}%n%{$fg_no_bold[white]%}@%m {$fg[green]%}[%*] %40<...<%B%~%b%<< %\# "
fi
...
export PATH="/sbin:/bin:/usr/sbin:/usr/bin:/usr/local/bin:/usr/local/sbin:/opt:/opt/bin:/opt/sbin:/home/roko__/bin:${PATH}"
...
#export GALLIUM_HUD=fps,requested-VRAM+requested-GTT,cpu:100
#export R600_GLSL130=1
#export R600_STREAMOUT=1
#export GALLIUM_HUD="cpu0+cpu1+cpu2+cpu3:100,cpu:100,fps;draw-calls,requested-VRAM+requested-GTT,pixels-rendered"
#export pp_jimenezmlaa=16
#export pp_jimenezmlaa_color=16
#export pp_cellshade=1
