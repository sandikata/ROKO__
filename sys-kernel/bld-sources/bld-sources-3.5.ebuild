...
src_compile() {
	...
	genkernel --kernel-config="config-desktop-`uname -m`-3.5" --no-save-config --loglevel=5 --kernname="${PN}" --kerneldir="$S" --tempdir=${WORKDIR}/build --makeopts="${MAKEOPTS}" --firmware-dir=${WORKDIR}/out/lib/firmware --cachedir="${T}/cache" --tempdir="${T}/twork" --logfile="${WORKDIR}/genkernel.log" --bootdir="${WORKDIR}/out/boot" --lvm --luks --iscsi --module-prefix="${WORKDIR}/out" all || die "genkernel failed"
}
