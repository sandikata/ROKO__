...
SLOT="stable"
KEYWORDS="amd64"
IUSE="bcachefs +bore cacule high-hz +nest +latency prjc tt"
...
#	eapply "${FILESDIR}/${KV_MAJOR}.${KV_MINOR}/${KV_MAJOR}.${KV_MINOR}-amd-pstate-epp-enhancement.patch"
	...
	if use bcachefs; then
		eapply "${FILESDIR}/${KV_MAJOR}.${KV_MINOR}/${KV_MAJOR}.${KV_MINOR}-bcachefs.patch"
	fi
	...
	if use nest; then
		eapply "${FILESDIR}/${KV_MAJOR}.${KV_MINOR}/${KV_MAJOR}.${KV_MINOR}-NEST-global-cachy.patch"
	fi
