...
SRC_URI="${P%_p*}-eter${PV#*_p}gentoo.i586.tar.bz2"

LICENSE="WINE@Etersoft-Local"
SLOT="0"
KEYWORDS="-* ~x86 ~amd64"
...
RDEPEND=">=media-libs/freetype-2.0.0
	...
	>=app-emulation/wine-etersoft-public-${PV%_p*}_p11
...
pkg_nofetch() {
	einfo "Please download the appropriate WINE@Etersoft-Local archive	${P%_p*}-eter${PV#*_p}gentoo.i586.tar.bz2"
...
src_install() {
	...
	use amd64 && { dodir /usr/lib32
					dodir /usr/lib
					cp "${WORKDIR}"/usr/lib/libwine-etersoft* "${D}"/usr/lib/ || die "cp eter-lib";
					cp -R "${WORKDIR}"/usr/lib/wine "${D}"/usr/lib32/ || die "cp libs"; }
